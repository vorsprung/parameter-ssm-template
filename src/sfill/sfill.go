...
func (p *Para) setup() {
	...
	if err != nil { panic("yike") }
	p.s = ssm.New(sess)
}


    // take a base path for the AWS Parameter store and return a map of the key/values
...
//returns a matcher function which checks that strings are valid
...
//ensure path parameters never have trailing /
...
func (p Para) Delete(path string) error {
    ...
    // load a file of lines into a string array
